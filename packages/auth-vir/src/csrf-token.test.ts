...
describe(getCurrentCsrfToken.name, () => {
    it('can override the localstorage key', () => {
        ...
        assert.strictEquals(
            getCurrentCsrfToken({localStorage, csrfHeaderName: mockKey}).csrfToken?.token,
            mockCsrfToken.token,
        );
        wipeCurrentCsrfToken({localStorage, csrfHeaderName: mockKey});
        assert.isUndefined(getCurrentCsrfToken({localStorage, csrfHeaderName: mockKey}).csrfToken);
    });
    it('uses prefix to generate header name', () => {
        ...
        wipeCurrentCsrfToken({localStorage, ...testCsrfOption});
        assert.isUndefined(getCurrentCsrfToken({localStorage, ...testCsrfOption}).csrfToken);
    });
});
